  ...
  class ManagerTest < Test::Unit::TestCase
    ...
    def setup
      ...
      @manager.source_configurations :data_base do |c|
        c.source = {adapter: 'sqlite3', database: 'test/files/db/tenants.sqlite3'}
    ...
    def test_tenants
      @manager.add_tenant('foo', {adapter: 'sqlite3', database: 'test/files/db/foo.sqlite3'})
      @manager.add_tenant('bar', {adapter: 'sqlite3', database: 'test/files/db/bar.sqlite3'})

      assert       @manager.tenants.key?('foo')
      assert_equal @manager.tenants['foo']['adapter'],  'sqlite3'
      assert_equal @manager.tenants['foo']['database'], 'test/files/db/foo.sqlite3'

      assert       @manager.tenants.key?('bar')
      assert_equal @manager.tenants['bar']['adapter'],  'sqlite3'
      assert_equal @manager.tenants['bar']['database'], 'test/files/db/bar.sqlite3'
    end
    ...
    def test_tenant
      ...
      assert_equal @manager.tenant('foo')['database'], 'test/files/db/foo.sqlite3'
    end
    ...
    def test_add_tenant
      assert @manager.add_tenant('foo', {adapter: 'sqlite3', database: 'test/filesdb/foo.sqlite3'})
    end
    ...
    def test_update_tenant
      ...
      assert @manager.update_tenant('foo', {adapter: 'sqlite3', database: 'test/files/db/bar.sqlite3'})
